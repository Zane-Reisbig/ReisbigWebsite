<template>
  <div id="main-content">
    <PageHeader />
    <div id="container-index">
      <p>
        My name is Zane Reisbig. I am a {{ getAgeOfMe() }} year old software
        developer from Michigan.
      </p>
      <p>
        I have made a few projects, you can check them out on the Nav-Bar up
        top.
      </p>
      <p>
        Or check out my github to see everything, not just the web projects!
      </p>
      <div id="email-section">
        <p>
          <a href="mailto:zanedreisbig@yahoo.com?subject=Reisbig.io Link"
            >zanedreisbig@yahoo.com</a
          >
        </p>
        <p id="email-after">Opens Mail Client</p>
      </div>
      <div id="logo-container">
        <a
          href="https://github.com/SRGTxTwinkie"
          alt="Link to my GitHub"
          target="_blank"
        >
          <img
            id="github-logo"
            src="https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png"
          />
        </a>
        <a
          href="https://www.linkedin.com/in/zane-reisbig-328119200/"
          alt="Link to my LinkedIn"
          target="_blank"
        >
          <svg
            id="linkedin-logo"
            data-name="Group 1282"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 76.624 65.326"
          >
            <path
              id="Path_2525"
              data-name="Path 2525"
              d="M1165,274.515a1.2,1.2,0,0,0,1.21-1.269c0-.9-.543-1.33-1.657-1.33h-1.8v4.712h.677v-2.054h.832l.019.025,1.291,2.029h.724l-1.389-2.1Zm-.783-.472h-.785V272.45h.995c.514,0,1.1.084,1.1.757,0,.774-.593.836-1.314.836"
              transform="translate(-1092.136 -213.406)"
              fill="#0a66c2"
            />
            <path
              id="Path_2520"
              data-name="Path 2520"
              d="M958.98,112.559h-9.6V97.525c0-3.585-.064-8.2-4.993-8.2-5,0-5.765,3.906-5.765,7.939v15.294h-9.6V81.642h9.216v4.225h.129a10.1,10.1,0,0,1,9.093-4.994c9.73,0,11.524,6.4,11.524,14.726ZM918.19,77.416a5.571,5.571,0,1,1,5.57-5.572,5.571,5.571,0,0,1-5.57,5.572m4.8,35.143h-9.61V81.642h9.61Zm40.776-55.2h-55.21a4.728,4.728,0,0,0-4.781,4.67v55.439a4.731,4.731,0,0,0,4.781,4.675h55.21a4.741,4.741,0,0,0,4.8-4.675V62.025a4.738,4.738,0,0,0-4.8-4.67"
              transform="translate(-903.776 -57.355)"
              fill="#0a66c2"
            />
            <path
              id="Path_2526"
              data-name="Path 2526"
              d="M1156.525,264.22a4.418,4.418,0,1,0,.085,0h-.085m0,8.33a3.874,3.874,0,1,1,3.809-3.938c0,.022,0,.043,0,.065a3.791,3.791,0,0,1-3.708,3.871h-.1"
              transform="translate(-1084.362 -207.809)"
              fill="#0a66c2"
            />
          </svg>
        </a>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import Vue from 'vue'

export default Vue.extend({
  name: 'IndexPage',
  data() {
    return {}
  },
  methods: {
    getAgeOfMe() {
      const today = new Date()
      const birthDate = new Date('2000-08-13')
      let age = today.getFullYear() - birthDate.getFullYear()
      const m = today.getMonth() - birthDate.getMonth()
      if (m < 0 || (m === 0 && today.getDate() < birthDate.getDate())) {
        age--
      }
      return age
    },
  },
})
</script>
<style scoped>
body {
  overflow: hidden !important;
}

#email-section {
  margin-top: 20px !important;
}

#email-section a {
  color: #0a66c2;
}

#email-after {
  opacity: 0;
  text-align: center;
  transition: all 0.5s ease-in-out;
}

#email-section:hover #email-after {
  opacity: 1;
}

#github-logo {
  border-radius: 20%;
  display: inline-block;
  opacity: 0.5;
}

#linkedin-logo {
  display: inline-block;
  margin-left: 50px;
  opacity: 0.5;
  transform: scale(1.2);
}

#linkedin-logo,
#github-logo {
  width: 100px;
  height: 100px;
}

#linkedin-logo:hover,
#github-logo:hover {
  opacity: 1;
}

#logo-container {
  display: flex;
  justify-content: center;
  align-items: center;
  margin-top: 20px;
}

#container-index {
  display: flex;
  justify-content: center;
  align-items: center;
  flex-direction: column;
  height: 100vh;
  font-size: 1.3em;
}

#container-index p {
  color: white;
  margin: 0 10px;
}

#main-content {
  background-color: #141414;
}

@media only screen and (max-width: 800px) {
  #container-index {
    height: 100vh;
  }
}
</style>
